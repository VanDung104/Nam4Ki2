...
def split_list(numbers, num_workers):
    chunk_size = (len(numbers) + num_workers - 1) // num_workers  # Làm tròn lên
    return [numbers[i*chunk_size : (i+1)*chunk_size] for i in range(min(num_workers, len(numbers)))]
...
if __name__ == "__main__":
    # Cấu hình
    NUMBERS = list(range(1, 10**6+1))  # Tăng lên 10 triệu số
    NUM_PROCESSES = 4
    NUM_THREADS = 4
